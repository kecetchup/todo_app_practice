...
public class JwtAuthenticationFilter extends OncePerRequestFilter { //매 요청마다 한 번만 실행되는 필터를 정의
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        String token = resolveToken(request); //요청 헤더에서 JWT를 추출
        ...
        if (token != null && jwtTokenProvider.validateToken(token)) { //추출한 토큰이 유효한지 검증
            ...
            JwtAuthenticationToken authentication = new JwtAuthenticationToken(userDetails, null, userDetails.getAuthorities()); //객체 생성
